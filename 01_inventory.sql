...
-- createdAt staggered to match recordDate order - SignificantMovementInInventoryInLastNTimes()
...
VALUES
(501, 'Significant Movement In Inventory (Test)', 'EWS-INV-TEST',
 'Stock value dropped >30% vs average over last N months', 2, CURRENT_DATE, (CURRENT_DATE + INTERVAL '5 years')::date,
 $j$
 {"name":"Significant Movement In Inventory (Test)","code":"EWS-INV-TEST","ruleType":"EWS_BASED","alertHierarchyId":1,
  "ruleSeverity":"MEDIUM","alertGenerationToClassificationTat":7,"action":[],
  "topCondition":"AND",
  "conditions":[{"Order":1,"name":"Significant Movement In Inventory",
   "function":"significantMovementInInventoryInLastNMonths","isAdvanceField":true,
...
FROM "FRM_RULE" r WHERE r.id = 501
ON CONFLICT DO NOTHING;
